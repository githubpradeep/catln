...
desClassDecl (inheritPath, _) (className, classVars) subStatements (Relative path) = ([], (H.empty, H.singleton (inheritPath ++ "/" ++ path) (False, classVars, [], desObjDocComment subStatements, inheritPath ++ "/" ++ path)), [])
desClassDecl _ (className, classVars) subStatements (Absolute path) = ([], (H.empty, H.singleton path (False, classVars, [], desObjDocComment subStatements, path)), [])
